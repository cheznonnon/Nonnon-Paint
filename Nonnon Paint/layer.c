...
n_paint_layer_ini_main( n_ini *ini, BOOL is_read )
{
	...
	n_type_int i = n_paint->layer_count - 1;
	n_posix_loop
	{
		...
		if ( is_read )
		{
			...
			n_string_copy( str, n_paint->layer_data[ i ].name );
...
//NSLog( @"#%lld : %d %d %d", i, visible, percent, blur );
			...
			n_paint->layer_data[ i ].visible = visible;
			n_paint->layer_data[ i ].percent = percent;
			n_paint->layer_data[ i ].blur    = blur;
			n_paint->layer_data[ i ].blend   = (n_type_real) percent * 0.01;
		} else {
			n_posix_char str[ N_PAINT_LAYER_CCH ];
...
//NSLog( @"ini_write() : %lld : %s", i, paint->layer_data[ i ].name );
			...
			int     blur = n_paint->layer_data[ i ].blur;
	...
	if ( is_read )
	{
		...
		n_ini_key_add_int( ini, N_PAINT_LAYER_INI_SECTION, N_PAINT_LAYER_INI_INDEX , (int) n_paint->layer_index );
		n_ini_key_add_int( ini, N_PAINT_LAYER_INI_SECTION, N_PAINT_LAYER_INI_NUMBER, (int) n_paint->layer_count );
	}
...
void
n_paint_layer_config_default_single( n_type_int i )
{

	n_paint->layer_data[ i ].visible = TRUE;
	n_paint->layer_data[ i ].percent = 100;
	n_paint->layer_data[ i ].blend   = 1.0;
	n_paint->layer_data[ i ].blur    =   0;
...
n_paint_layer_text_set( n_type_int i, n_posix_char *str )
{
	...
	{
		n_txt_add( &n_paint->layer_txt, i, str );
	} else {
		n_txt_set( &n_paint->layer_txt, i, str );
	}
//NSLog( @"%s", n_txt_get( &n_paint->layer_txt, i ) );


	n_posix_char deco[ 2 ] = { 0, 0 };

	if ( n_paint->layer_data[ i ].visible )
	{
		deco[ 0 ] = 'B';
	} else {
		deco[ 0 ] = ' ';
	...
	n_txt_set( &n_paint->layer_decoration_txt, i, deco );
...
//NSLog( @"%s %s", n_paint->layer_data[ i ].name, str );
	...
	n_txt_mod( &n_paint->layer_decoration_txt, i, deco );
...
n_bmp_layer_free( void )
{
	...
	n_posix_loop
	{
		...
		n_bmp_free( &n_paint->layer_data[ i ].bmp_data );
		n_bmp_free( &n_paint->layer_data[ i ].bmp_grab );

		n_bmp_free( &n_paint->layer_data[ i ].bmp_blur );
...
void
n_bmp_layer_turn_off( void )
{

	n_paint->layer_onoff = FALSE;

	n_paint->layer_load_as_layer = FALSE;

	n_bmp_layer_free();
	//n_memory_free( n_paint->layer_data );
...
//return TRUE;
	...
	BOOL ret = n_txt_load_utf8( (void*) &ini_sniffer, name_ini );
//NSLog( @"%d", ret );
	if ( ini_sniffer.readonly ) { ret = TRUE; }
...
//NSLog( @"%d", ret ); ret = TRUE;
	if ( ret == FALSE )
	{
		...
		n_ini_free( &n_paint->layer_ini );
		n_memory_copy( &ini_sniffer, &n_paint->layer_ini, sizeof( n_ini ) );

		n_paint_layer_ini_read( &n_paint->layer_ini );

		n_bmp_layer_free();
		n_memory_free( n_paint->layer_data );

		n_paint->filename = [NSString stringWithFormat:@"%s.lyr", name];

		n_txt_new( &n_paint->layer_txt );
		n_txt_new( &n_paint->layer_decoration_txt );
...
//NSLog( @"Layer Cout : %d", paint->layer_count );
		...
		n_type_real step = 100.0 / n_paint->layer_count;
		n_type_real pc   = 0;
		...
		n_posix_loop
		{
			...
			if ( n_bmp_error( &n_paint->layer_data[ i ].bmp_data ) )
			{
				n_type_gfx sx = N_BMP_SX( &n_paint->layer_data[ 0 ].bmp_data );
				n_type_gfx sy = N_BMP_SY( &n_paint->layer_data[ 0 ].bmp_data );

				n_bmp_new_fast( &n_paint->layer_data[ i ].bmp_data, sx, sy );
				n_bmp_flush( &n_paint->layer_data[ i ].bmp_data, n_bmp_white_invisible );
			} else {
			...
			int     blur = n_ini_value_int( &n_paint->layer_ini, sec, N_PAINT_LAYER_INI_BLUR   ,    0 );
			...
			n_ini_value_str( &n_paint->layer_ini, sec, N_PAINT_LAYER_INI_NAME, N_STRING_EMPTY, nam, N_PAINT_LAYER_CCH );

			n_paint_layer_text_set( i, nam );

			/* update progress and throttle UI updates */
			pc += step;
			if ( pc >= 100 ) { pc = 100; }

			n_paint->layer_load_onoff   = TRUE;
			n_paint->layer_load_percent = pc;
			if ( n_bmp_ui_timer_once( &wait, 100 ) )
			{
		...
		n_paint->layer_index = n_ini_value_int( &n_paint->layer_ini, N_PAINT_LAYER_INI_SECTION, N_PAINT_LAYER_INI_INDEX, 0 );
		n_paint->layer_index = n_posix_minmax_n_type_int( 0, n_paint->layer_count - 1, n_paint->layer_index );
//NSLog( @"%d", paint->layer_index );
		...
		n_paint->layer_onoff = TRUE;

		n_paint->layer_load_as_layer = TRUE;
...
BOOL
n_paint_layer_save( const n_posix_char *cmdline )
{

	n_posix_char *name = n_string_path_carboncopy( cmdline );


	if ( FALSE == n_posix_stat_is_exist( name ) )
	{
	...
	if ( n_paint->layer_load_as_layer )
	{
		...
		n_type_gfx bmpsx = N_BMP_SX( &n_paint->bmp_data );
		n_type_gfx bmpsy = N_BMP_SY( &n_paint->bmp_data );
//NSLog( @"%d : %d %d", paint->layer_count, bmpsx, bmpsy );
		...
		n_posix_loop
		{
			if ( i == 0 )
			{
				n_bmp_carboncopy( &n_paint->bmp_data, &n_paint->layer_data[ i ].bmp_data );
			} else {
				n_bmp_new_fast( &n_paint->layer_data[ i ].bmp_data, bmpsx, bmpsy );
				n_bmp_flush( &n_paint->layer_data[ i ].bmp_data, n_bmp_white_invisible );
			}
		...
		n_posix_loop
		{
			...
			n_string_truncate( n_paint->layer_data[ i ].name );

			if ( i == 0 )
			{
				n_posix_strcat( n_paint->layer_data[ i ].name, n_posix_literal( "Background" ) );
			} else
			if ( i == ( n_paint->layer_count - 1 ) )
			{
				n_posix_strcat( n_paint->layer_data[ i ].name, n_posix_literal( "Top" ) );
			}
...
//n_posix_debug_literal( " %s ", ini_name );
	...
	{//break;
		...
		n_type_int cb_b = N_BMP_SIZE( &n_paint->layer_data[ i ].bmp_data );
		n_type_int cb_c = n_posix_min_n_type_int( cb_a, cb_b );

		if (
			( cb_a == cb_b )
			&&
			( n_memory_is_same( N_BMP_PTR( &bmp_check ), N_BMP_PTR( &n_paint->layer_data[ i ].bmp_data ), cb_c ) )
			&&
			( N_BMP_SX( &bmp_check ) == N_BMP_SX( &n_paint->layer_data[ i ].bmp_data ) )
			&&
			( N_BMP_SY( &bmp_check ) == N_BMP_SY( &n_paint->layer_data[ i ].bmp_data ) )
		)
		{
...
//NSLog( @"Not the Same : %lld", i );
		...
		if ( n_bmp_ui_timer_once( &wait, 200 ) )
		{
			n_paint->layer_load_onoff   = TRUE;
			n_paint->layer_load_percent = pc;
			[n_paint_global display];
	...
	n_ini_save( &n_paint->layer_ini, ini_name );
...
n_paint_layer_bmp_alpha( int mode )
{
		...
		n_posix_loop
		{
			...
			if ( n_paint->layer_data[ i ].visible )
			{
				...
				if ( n_paint->grabber_mode == N_PAINT_GRABBER_NEUTRAL )
				{
					...
					{
						n_bmp_alpha_visible( &n_paint->layer_data[ i ].bmp_data );
					} else
					if ( mode == N_PAINT_FILTER_ALPHA_REV )
					{
						n_bmp_alpha_reverse( &n_paint->layer_data[ i ].bmp_data );
					}// else
				} else {
					if ( mode == N_PAINT_FILTER_ALPHA_CLR )
					{
						n_bmp_alpha_visible( &n_paint->layer_data[ i ].bmp_grab );
					} else
					if ( mode == N_PAINT_FILTER_ALPHA_REV )
					{
						n_bmp_alpha_reverse( &n_paint->layer_data[ i ].bmp_grab );
					}// else
				}
			}
	...
	if ( n_paint->layer_onoff )
	{
		...
		if ( n_paint->layer_data[ y ].visible )
		{
			...
			if ( n_paint->grabber_mode == N_PAINT_GRABBER_NEUTRAL )
			{
				if ( mode == N_PAINT_FILTER_ALPHA_CLR )
				{
					n_bmp_alpha_visible( n_paint->pen_bmp_data );
				} else
				if ( mode == N_PAINT_FILTER_ALPHA_REV )
				{
					n_bmp_alpha_reverse( n_paint->pen_bmp_data );
				}// else
			} else {
				if ( mode == N_PAINT_FILTER_ALPHA_CLR )
				{
					n_bmp_alpha_visible( n_paint->pen_bmp_grab );
				} else
				if ( mode == N_PAINT_FILTER_ALPHA_REV )
				{
					n_bmp_alpha_reverse( n_paint->pen_bmp_grab );
				}// else
			}
		}
...
#define n_paint_layer_blur_pixel_cached(      i, x,y ) n_paint_layer_blur_pixel_cached_internal( i, x,y,  NO )
#define n_paint_layer_blur_pixel_cached_fast( i, x,y ) n_paint_layer_blur_pixel_cached_internal( i, x,y, YES )

// internal
u32
n_paint_layer_blur_pixel_cached_internal( n_type_int index, n_type_gfx x, n_type_gfx y, BOOL is_fast )
{

	u32 ret;


	int blur = n_paint->layer_data[ index ].blur;

	if ( blur == 0 )
	{
		if ( is_fast )
		{
			n_bmp_ptr_get_fast( &n_paint->layer_data[ index ].bmp_data, x, y, &ret );
		} else {
			n_bmp_ptr_get     ( &n_paint->layer_data[ index ].bmp_data, x, y, &ret );
		}
	} else {
		u32  chk = 0;
		BOOL err = n_bmp_ptr_get( &n_paint->layer_data[ index ].bmp_blur, x,y, &chk );

		if ( err )
		{
			if ( blur == 1 )
			{
				ret = n_bmp_antialias_pixel( &n_paint->layer_data[ index ].bmp_data, x, y, 1.0 );
			} else {
				ret = n_bmp_blur_pixel( &n_paint->layer_data[ index ].bmp_data, x, y, blur, 1, 1.0 );
			}
		} else
		if ( chk == 0 )
		{
...
//ret = n_bmp_rgb_mac( 0,200,255 );
			...
			n_bmp_ptr_set_fast( &n_paint->layer_data[ index ].bmp_blur, x,y, ret );
		} else {
			ret = chk;
//ret = n_bmp_rgb_mac( 0,200,255 );
		}
...
//NSLog( @"n_paint_layer_blur_cache" );
	...
	n_bmp *bmp = &n_paint->layer_data[ index ].bmp_blur;
	...
	n_posix_loop
	{
		...
		n_posix_loop
		{
			n_type_gfx x = n_random_range( sx );
			n_type_gfx y = n_random_range( sy );

			n_paint_layer_blur_pixel_cached_fast( index, x,y );
...
n_paint_layer_grabber_pixel_get( n_bmp *grab, n_type_gfx tx, n_type_gfx ty, u32 picked, u32 *before, u32 *after, n_type_real blend, n_type_int i )
{
	...
	if ( n_paint->grabber_mode == N_PAINT_GRABBER_NEUTRAL )
	{
		...
		color_b = n_bmp_antialias_pixel( n_paint->pen_bmp_data, tx,ty, blend );
		color_a = picked;
		...
		{
			color_b = n_paint_layer_blur_pixel_fast( &n_paint->layer_data[ i ].bmp_grab, gx, gy, n_paint->layer_data[ i ].blur );
		} else {
			color_b = n_paint_layer_blur_pixel     ( &n_paint->layer_data[ i ].bmp_data, tx, ty, n_paint->layer_data[ i ].blur );
		}
		...
		{
			n_bmp_ptr_get( n_paint->pen_bmp_data, tx,ty, &color_a );
		} else {
			color_a = picked;
		}
...
n_paint_layer_canvas_grabber_pixel( n_bmp *grab, n_type_gfx tx, n_type_gfx ty, u32 color, n_type_int i )
{
	...
	if ( n_paint->grabber_per_pixel_alpha_onoff )
	{
		...
		n_bmp *data = &n_paint->layer_data[ i ].bmp_data;
		...
		{
			color_grab = n_bmp_composite_pixel_fast( grab, data, gx,gy, tx,ty, FALSE,FALSE, TRUE, TRUE, 0.0, NULL );
		}
	}
	...
	{
		color_grab = n_bmp_blend_pixel( color_grab, color, n_paint->grabber_blend_ratio );
	}
...
u32
n_bmp_layer_ptr_get( n_paint_layer *p, n_type_gfx tx, n_type_gfx ty, BOOL use_color_bg, u32 color_bg, BOOL is_ui )
{
...
//return n_bmp_rgb( 0,200,255 );
	...
	{
		color_prev = n_paint_layer_blur_pixel_cached_fast( 0, tx,ty );
	} else {
...
//return n_bmp_rgb( 255,0,200 );
	...
	if ( is_ui )
	{
		n_bmp_ptr_get( &n_paint->layer_cache_bmp_data, tx,ty, &check );
	}
	...
	n_type_int     i = 0;
	n_type_int count = n_paint->layer_count;


	BOOL cache_onoff = is_ui;
	if ( n_paint->layer_whole_grab_onoff ) { cache_onoff = FALSE; }
...
//NSLog( @"%d %d", ( NULL == N_BMP_PTR( &paint->layer_cache_bmp_pre ) ), ( NULL == N_BMP_PTR( &paint->layer_cache_bmp_pst ) ) );
	...
	if ( NULL == N_BMP_PTR( &n_paint->layer_cache_bmp_pre ) )
	{
		cache_onoff_pre = FALSE;
	}

	if ( NULL == N_BMP_PTR( &n_paint->layer_cache_bmp_pst ) )
	{
	...
	if ( cache_onoff_pre )
	{
		n_bmp_ptr_get_fast( &n_paint->layer_cache_bmp_pre, tx,ty, &check );
	...
	{//break;
		...
		if ( p[ i ].visible )
		{
			...
			if ( i == 0 )
			{
				...
				{
					color = n_paint_layer_blur_pixel_cached_fast( i, tx,ty );
				} else {
					color = n_paint_layer_blur_pixel_fast( &p[ i ].bmp_data, tx, ty, p[ i ].blur );
				}
			}

			if ( ( is_ui )&&( n_paint->grabber_mode ) )
			{
				color = n_paint_layer_canvas_grabber_pixel( &n_paint->layer_data[ i ].bmp_grab, tx, ty, color, i );
...
//if ( n_win_is_input( VK_LBUTTON ) ) { ret = n_bmp_rgb( 0,200,255 ); }
			...
			if ( ( cache_onoff_pst )&&( i > n_paint->layer_index ) )
			{
				color_post = n_bmp_composite_pixel_postprocess( color_post, color, n_bmp_a( color_post ), TRUE, TRUE, TRUE, 1.0 - p[ i ].blend, NULL );
			}

		} else
		if ( n_paint->layer_whole_preview_onoff )
		{
...
//color = n_bmp_rgb( 0,200,255 );
		...
		if ( ( cache_onoff_pre )&&( i == ( n_paint->layer_index - 1 ) ) )
		{
			n_bmp_ptr_set_fast( &n_paint->layer_cache_bmp_pre, tx,ty, ret );
		}
		...
		if ( ( cache_onoff_pst )&&( i > n_paint->layer_index ) )
		{
			n_bmp_ptr_get_fast( &n_paint->layer_cache_bmp_pst, tx,ty, &check );
			if ( check )
			{
				ret = n_bmp_composite_pixel_postprocess( color_prev, check, n_bmp_a( color_prev ), TRUE, TRUE, TRUE, 1.0 - p[ i ].blend, NULL );
				break;
			}
		}
		...
		if ( i >= count )
		{
			if ( cache_onoff_pst ) { n_bmp_ptr_set_fast( &n_paint->layer_cache_bmp_pst, tx,ty, color_post ); }
			break;
		}
	}


	if ( NULL != N_BMP_PTR( &n_paint->layer_cache_bmp_data ) )
	{
		n_bmp_ptr_set_fast( &n_paint->layer_cache_bmp_data, tx,ty, ret );
	}
...
// internal
BOOL
n_paint_layer_swap( n_type_int y, BOOL is_up )
{
	...
	if ( is_up )
	{
		...
		n_paint_layer       data_tmp = n_paint->layer_data[ y - 1 ];
		n_paint->layer_data[ y - 1 ] = n_paint->layer_data[ y     ];
		n_paint->layer_data[ y     ] = data_tmp;

		n_posix_char           *line_tmp = n_paint->layer_txt.line[ y - 1 ];
		n_paint->layer_txt.line[ y - 1 ] = n_paint->layer_txt.line[ y     ];
		n_paint->layer_txt.line[ y     ] = line_tmp;

		n_posix_char                      *deco_tmp = n_paint->layer_decoration_txt.line[ y - 1 ];
		n_paint->layer_decoration_txt.line[ y - 1 ] = n_paint->layer_decoration_txt.line[ y     ];
		n_paint->layer_decoration_txt.line[ y     ] = deco_tmp;
		...
		if ( y >= ( n_paint->layer_count - 1 ) ) { return ret; }
...
n_paint_layer_visibility_onoff_all( BOOL onoff )
{
	...
	n_posix_loop
	{
		...
		n_posix_char *nam = n_txt_get( &n_paint->layer_txt, i );
		...
		if ( onoff )
		{
			...
			n_paint->layer_data[ i ].visible = FALSE;
			n_paint_layer_text_mod( i, nam );
		}
...
//NSLog( @"n_paint_layer_info()" );
	...
	int byte = n_paint->layer_count * sizeof( int );
	...
	n_posix_loop
	{
		...
		u32 color; n_bmp_ptr_get( &n_paint->layer_data[ i ].bmp_data, tx, ty, &color );
//NSLog( @"#%lld : %08x", i, color );
	...
	{//break;
		...
		n_posix_char *str = n_txt_get( &n_paint->layer_decoration_txt, i );
