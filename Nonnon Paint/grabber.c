...
n_paint_grabber_resync( n_type_gfx x, n_type_gfx y, n_type_gfx sx, n_type_gfx sy )
{
	...
	if ( p->grabber_frame_mode == N_PAINT_GRABBER_FRAME_MODE_EDGE )
	{
		...
		area = n_paint_grabber_frame_edge_redraw_area_get( p );

	} else {
		extern n_type_gfx n_paint_grabber_frame_pixel_redraw_area_get( n_paint *paint );
		area = n_paint_grabber_frame_pixel_redraw_area_get( p );
	}
...
//[n_paint_global display_optimized]; return;
	...
	[n_paint_global displayRect:paint->grabber_tooltip_rect_drawn];
...
// internal
void
n_paint_grabber_select_drop( n_bmp *b, n_posix_bool is_zero, n_posix_bool redraw )
{
...
//u32 tick = n_posix_tickcount();
	...
	{//break;
		...
		if ( data_onoff )
		{
			n_paint_grabber_filter_go( &paint->layer_data[ i ].bmp_data, filter_type );
		}

		if ( grab_onoff )
		{
			n_paint_grabber_filter_go( &paint->layer_data[ i ].bmp_grab, filter_type );
		}
...
//NSLog( @"N_PAINT_FILTER_SCALE_BIG" );
	...
	if ( filter_type == N_PAINT_FILTER_ROTATE_L  )
	{
		...
		CGFloat tmp = p->scroll.x;
		p->scroll.x = p->scroll.y;
		p->scroll.y = tmp;
	} else
...
n_paint_grabber_select_all( void )
{
	...
	extern void n_paint_grabber_frame_reset( n_paint* );
	n_paint_grabber_frame_reset( p );
...
void
NonnonPaintGrabber_mouseDown( n_paint *paint, NSEvent *theEvent )
{
...
//n_win_text_set_literal( hwnd, "Grabber : DRAG_OK : WM_LBUTTONDBLCLK" );
	...
	if ( [theEvent clickCount] == 1 )
	{
		...
		if ( paint->grabber_mode == N_PAINT_GRABBER_NEUTRAL )
		{
			...
			paint->grabber_delay_onoff = TRUE;
			paint->grabber_delay_timer = n_posix_tickcount();
...

			{
				...
				const n_type_gfx maxsx = N_BMP_SX( paint->pen_bmp_data ) - 1;
				const n_type_gfx maxsy = N_BMP_SY( paint->pen_bmp_data ) - 1;
			...
			paint->grabber_delayed_x = x;
			paint->grabber_delayed_y = y;
...
//NSLog( @"n_paint_frame_anim_lock : N_PAINT_GRABBER_DRAG_OK" );
			...
			if ( FALSE == ( theEvent.modifierFlags & NSEventModifierFlagCommand ) )
			{
				...
				paint->grabber_drag_ox = canvas_x - x;
				paint->grabber_drag_oy = canvas_y - y;
				...
				if ( theEvent.modifierFlags & NSEventModifierFlagOption )
				{
					...
					paint->grabber_stretch_ratio = (n_type_real) sx / sy;
				}
				...
				{
					paint->grabber_stretch_bmp = n_memory_new( sizeof( n_bmp ) * paint->layer_count );
					n_paint_grabber_wholegrb_stretch( N_PAINT_GRABBER_WHOLEGRAB_STRETCH_BACKUP, 0,0 );
				} else
				if ( paint->layer_onoff )
				{
					paint->grabber_stretch_bmp = n_memory_new( sizeof( n_bmp ) );
					n_bmp_carboncopy( paint->pen_bmp_grab, paint->grabber_stretch_bmp );
				} else {
					paint->grabber_stretch_bmp = n_memory_new( sizeof( n_bmp ) );
					n_bmp_carboncopy( paint->pen_bmp_grab, paint->grabber_stretch_bmp );
				}

				paint->grabber_stretch_px = x;
				paint->grabber_stretch_py = y;
				...
				paint->grabber_stretch_sx = sx;
				paint->grabber_stretch_sy = sy;
				paint->grabber_stretch_cx =  x;
				paint->grabber_stretch_cy =  y;

//NSLog( @"%d", ( x + sx ) - canvas_x );
				paint->grabber_stretch_ox_l = paint->grabber_stretch_px - canvas_x;
				paint->grabber_stretch_oy_u = paint->grabber_stretch_py - canvas_y;
				paint->grabber_stretch_ox_r = ( paint->grabber_stretch_px + sx ) - canvas_x;
				paint->grabber_stretch_oy_d = ( paint->grabber_stretch_py + sy ) - canvas_y;

				paint->grabber_stretch_start_x = canvas_x;
				paint->grabber_stretch_start_y = canvas_y;

//NSLog( @"%d", paint->grabber_mode );
				extern void NonnonPaintGrabber_mouseDragged( n_paint *paint );
...
void
NonnonPaintGrabber_mouseDragged( n_paint *paint )
{

	if ( paint->tooltype != N_PAINT_TOOL_TYPE_GRABBER ) { return; }


	if ( paint->grabber_delay_onoff )
	{

		if ( n_posix_false == n_game_timer( &paint->grabber_delay_timer, 100 ) ) { return; }


		paint->grabber_delay_onoff = FALSE;


		n_type_gfx x = paint->grabber_delayed_x;
		n_type_gfx y = paint->grabber_delayed_y;
		n_type_gfx s = 0;
	...
	if ( paint->grabber_mode == N_PAINT_GRABBER_SELECTING )
	{
		...
		sx = abs( paint->grabber_drag_fx - x ) + 1;
		sy = abs( paint->grabber_drag_fy - y ) + 1;


		// Phase 3 : position : top-left

		fx = x = n_posix_min_n_type_gfx( paint->grabber_drag_fx, x );
		fy = y = n_posix_min_n_type_gfx( paint->grabber_drag_fy, y );
...
//n_win_text_set_literal( hwnd, "Grabber : DRAGGING : WM_MOUSEMOVE" );
		...
		x -= paint->grabber_drag_ox;
		y -= paint->grabber_drag_oy;
...
//[n_paint_global display_optimized];
	...
	)
	{
		...
		if ( ( paint->grabber_stretch_start_x > paint->grabber_stretch_cx )&&( paint->grabber_stretch_start_y > paint->grabber_stretch_cy ) )
		{
			// [!] : Bottom Right
			x += paint->grabber_stretch_ox_r;
			y += paint->grabber_stretch_oy_d;
		} else
		if ( ( paint->grabber_stretch_start_x < paint->grabber_stretch_cx )&&( paint->grabber_stretch_start_y < paint->grabber_stretch_cy ) )
		{
			// [!] : Top Left
			x += paint->grabber_stretch_ox_l;
			y += paint->grabber_stretch_oy_u;
		} else
		if ( ( paint->grabber_stretch_start_x < paint->grabber_stretch_cx )&&( paint->grabber_stretch_start_y > paint->grabber_stretch_cy ) )
		{
			// [!] : Bottom Left
		...
		{
			sy = abs( paint->grabber_stretch_cy - y ) * 2;
			sx = (n_type_gfx) ( (n_type_real) sy * paint->grabber_stretch_ratio );
		} else {
			sx = abs( paint->grabber_stretch_cx - x ) * 2;
			sy = abs( paint->grabber_stretch_cy - y ) * 2;
		}
...
//n_win_hwndprintf_literal( hwnd, "%d : %d", (int) sx, (int) sy );
		...
		x = paint->grabber_stretch_cx - ( sx / 2 );
		y = paint->grabber_stretch_cy - ( sy / 2 );
		...
		if ( ( ratio_x * paint->grabber_stretch_sx ) == 0 ) { return; }
		if ( ( ratio_y * paint->grabber_stretch_sy ) == 0 ) { return; }
		...
		if ( paint->layer_whole_grab_onoff )
		{
			...
			n_posix_loop
			{
				...
				sx = n_posix_max_n_type_gfx( sx, N_BMP_SX( &paint->layer_data[ i ].bmp_grab ) );
				sy = n_posix_max_n_type_gfx( sy, N_BMP_SY( &paint->layer_data[ i ].bmp_grab ) );
		...
		if ( paint->layer_onoff )
		{
			...
			n_bmp b; n_bmp_carboncopy( paint->grabber_stretch_bmp, &b );

			n_bmp_resampler( &b, ratio_x,ratio_y );
			n_paint_grabber_select( &b, n_posix_false );
...
void
NonnonPaintGrabber_mouseUp( n_paint *paint )
{
...
//n_win_text_set_literal( hwnd, "Grabber : SELECTING : WM_LBUTTONUP" );
		...
		paint->grabber_is_resel = n_posix_false;
...
//NSLog( @"%@", name );
			...
			n_posix_bool n_bmp_carboncopy_onoff = n_posix_true;

			if ( paint->layer_whole_save_onoff )
			{
				n_bmp_carboncopy_onoff = n_posix_false;
			}
			...
			n_posix_bool ret = n_paint_save_png( paint->pen_bmp_grab, str, n_bmp_carboncopy_onoff );
			if ( ret )
			{
//NSLog( @"1" );
				...
				//[panel beginSheetModalForWindow:paint->nswindow[ 0 ] completionHandler:^(NSInteger result)
				{
					if ( result == NSModalResponseOK )
					{
						...
						n_paint_save_png( paint->pen_bmp_grab, str2, n_bmp_carboncopy_onoff );
...
//n_win_text_set_literal( hwnd, "Grabber : STRETCHING : WM_LBUTTONUP" );
		...
		paint->xmouse_stop = TRUE;
		BOOL ret = n_mac_window_dialog_yesno( "Really OK?" );
		paint->xmouse_stop = FALSE;
		...
		if ( ret )
		{
			...
			x = paint->grabber_stretch_px;
			y = paint->grabber_stretch_py;
			...
			if ( paint->layer_onoff )
			{
				n_paint_grabber_select( paint->grabber_stretch_bmp, n_posix_true );
			} else {
				n_paint_grabber_select( paint->grabber_stretch_bmp, n_posix_true );
			}
		...
		if ( paint->layer_onoff )
		{
			n_bmp_free( paint->grabber_stretch_bmp );
		} else {
			n_bmp_free( paint->grabber_stretch_bmp );
		}

		n_memory_free( paint->grabber_stretch_bmp );
...
void
NonnonPaintGrabber_rightMouseDown( n_paint *paint )
{
...
void
NonnonPaintGrabber_keyDown_arrow( n_paint *paint, int keyCode )
{
//return;
	...
	case N_MAC_KEYCODE_ARROW_UP :
		...
		paint->grabber_is_key_input |= N_PAINT_GRABBER_INPUT_U;
	...
	case N_MAC_KEYCODE_ARROW_DOWN :
		...
		paint->grabber_is_key_input |= N_PAINT_GRABBER_INPUT_D;
	...
	case N_MAC_KEYCODE_ARROW_LEFT :
		...
		paint->grabber_is_key_input |= N_PAINT_GRABBER_INPUT_L;
	...
	case N_MAC_KEYCODE_ARROW_RIGHT:
		...
		paint->grabber_is_key_input |= N_PAINT_GRABBER_INPUT_R;
...
void
NonnonPaintGrabber_keyUp_arrow( n_paint *paint, int keyCode )
{
//return;
	...
	if ( paint->grabber_is_key_input == 0 )
	{
//NSLog( @"!" );
		paint->grabber_is_key_input_fade = TRUE;
		paint->grabber_is_key_input_tick = n_posix_tickcount();
	}
